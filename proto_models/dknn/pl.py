...
class PL(Distribution):
    ...
    def relaxed_sort(self, inp):
        ...
        if self.hard:
            ...
            b_idx = torch.arange(bsize).repeat([1, dim]).view(dim, bsize).transpose(
                dim0=1, dim1=0).flatten().type(LongTensor)
    ...
    def rsample(self, sample_shape, log_score=True):
        ...
        with torch.enable_grad():  # torch.distributions turns off autograd
            ...
            if not log_score:
                log_s_perturb = torch.log(self.scores.unsqueeze(0)) + sample_gumbel(
                    [n_samples, 1, self.n, 1])
            else:
...
if __name__ == '__main__':
    ...
    good_pm = torch.Tensor([[[0., 0., 1.], [1., 0., 0.], [0., 1., 0.]]])
    intermediate_pm = torch.Tensor([[[0., 0., 1.], [0., 1., 0.], [1., 0., 0.]]])
    bad_pm = torch.Tensor([[[0., 1., 0.], [1., 0., 0.], [0., 0., 1.]]])
    ...
    print(pl_dist_prior.log_prob(good_pm), pl_dist_prior.log_prob(intermediate_pm),
